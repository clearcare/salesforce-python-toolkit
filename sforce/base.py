...
Salesforce will use HTTPS.')
    ...
    def _handleResultTyping(self, result):
        ...
        return the single result, rather than [(SaveResult) {...}]:
            ...
            sendEmail()
        '''
        if self._strictResultTyping is False and len(result) == 1:
            return result[0]
        else:
            return result
    ...
    def _setHeaders(self, call=None):
        ...
        if call in ('convertLead', 'create', 'merge', 'process', 'undelete', 'update', 'upsert'):
        ...
        if self._callOptions is not None:
            if call in ('create', 'merge', 'queryAll', 'query', 'queryMore', 'retrieve', 'search', 'update', 'upsert', 'convertLead', 'login', 'delete', 'describeGlobal', 'describeLayout', 'describeTabs', 'describeSObject', 'describeSObjects', 'getDeleted', 'getUpdated', 'process', 'undelete', 'getServerTimestamp', 'getUserInfo', 'setPassword', 'resetPassword'):
        ...
        if call in ('create', 'merge', 'query', 'retrieve', 'update', 'upsert'):
            if self._mruHeader is not None:
                headers['MruHeader'] = self._mruHeader

        if call in ('convertLead', 'create', 'delete', 'describeGlobal', 'describeLayout', 'describeSObject', 'describeSObjects', 'describeTabs', 'merge', 'process', 'query', 'retrieve', 'search', 'undelete', 'update', 'upsert'):
